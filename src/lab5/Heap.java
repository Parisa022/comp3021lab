...
public class Heap<T extends Comparable<T>> {
    ...
    /**
     *
     * @return If size is 0, throw {@link IllegalStateException}. Otherwise, temporarily
    ...
    public T poll() {
    	...
    	T first = this.container.get(0);
    	this.container.set(0, this.container.get(size-1));
    ...
    /**
     * While the last element has a parent and is smaller than its parent, swap the two elements. 
     * Then, check again
    ...
    private void heapifyUp() {
    	...
    	while(this.hasParent(pos)) {
    		int par = this.getParentIndex(pos);
    		if (this.container.get(pos).compareTo(this.container.get(par)) < 0) {
    			swap(pos, par);
    			pos = par;
    		}
    		else {
